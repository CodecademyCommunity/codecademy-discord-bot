...
module.exports = {
  ...
  execute(msg, userid, fetch, con, sgMail) {
    ...
    const clientUsername = params.split(' ');
    ...
    if (clientUsername == 'cc!sendcode') {
      ...
      (async () => {
        const response = await fetch(
          `https://discuss.codecademy.com/users/${clientUsername[0]}/emails.json`,
          {
            method: 'get',
            headers: {
        ...
        if (email === undefined) {
          ...
          const sql = `INSERT INTO verifications (username, verify_id, expiration) VALUES ('${clientUsername[0]}', '${uuid}', '${expDateFormatted}')`;
